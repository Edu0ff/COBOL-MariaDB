...
-- Schema banco
-- -----------------------------------------------------
CREATE OR REPLACE DATABASE  `banco` ;
USE `banco` ;
-- -----------------------------------------------------
-- Table `banco`.`domicilio`
-- -----------------------------------------------------
CREATE OR REPLACE TABLE `banco`.`domicilio` (
...
-- Table `banco`.`cliente`
-- -----------------------------------------------------
CREATE OR REPLACE TABLE `banco`.`cliente` (
  ...
  `id_dom` INT UNSIGNED NULL COMMENT 'Clave primaria del domicilio',
...
-- Table `banco`.`medio`
-- -----------------------------------------------------
CREATE OR REPLACE TABLE `banco`.`medio` (
  `id_medio` INT UNSIGNED NOT NULL COMMENT 'Clave primaria del medio de pago',
  `tip_med` ENUM('C', 'T') NOT NULL COMMENT 'Tipo de medio: cuenta o tarjeta',
...
-- -----------------------------------------------------
CREATE OR REPLACE TABLE `banco`.`cliente_rln_cuenta` (
  `id_cliente` INT UNSIGNED NOT NULL COMMENT 'Clave primaria del cliente',
  `id_medio` INT UNSIGNED NOT NULL COMMENT 'Clave primaria de la cuenta',
  `tip_rln` ENUM('T', 'C', 'A') NOT NULL COMMENT 'Tipo de relación entre el cliente y la cuenta: titular, cotitular o autorizado',
...
-- Table `banco`.`tarjeta`
-- -----------------------------------------------------
CREATE OR REPLACE TABLE `banco`.`tarjeta` (
  ...
  `cred_tarjeta` DECIMAL(10,2) NOT NULL COMMENT 'Crédito de la tarjeta',
...
-- -----------------------------------------------------
CREATE OR REPLACE TABLE `banco`.`movimiento` (
  ...
  `importe_mov` DECIMAL(10,2) NOT NULL COMMENT 'Importe del  movimiento',
...
-- --------
DELIMITER //
CREATE OR REPLACE TRIGGER banco.actualiza_saldo AFTER INSERT ON banco.movimiento FOR EACH ROW
BEGIN
    DECLARE tipo_medio enum('C','T');
   
    SELECT tip_med
    INTO tipo_medio
    FROM banco.medio
    WHERE id_medio = NEW.id_medio;

    IF tipo_medio = 'C' THEN
        UPDATE banco.cuenta
        SET saldo_cuenta = saldo_cuenta + NEW.importe_mov
        WHERE id_cuenta = (SELECT id_cuenta FROM banco.medio WHERE id_medio = NEW.id_medio);
    ELSE
        UPDATE banco.tarjeta
        SET cred_tarjeta = cred_tarjeta + NEW.importe_mov
        WHERE id_tarjeta = (SELECT id_tarjeta FROM banco.medio WHERE id_medio = NEW.id_medio);
    END IF;
END;
DELIMITER ;
